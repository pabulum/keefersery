...
async function fetchAllCommits(repo: string, maxPages = 4): Promise<ApiCommit[]> {
  const all: ApiCommit[] = []
  for (let page = 1; page <= maxPages; page++) {
    const batch = await gh<ApiCommit[]>(`/repos/${repo}/commits?per_page=100&page=${page}`)
...
async function loadCache(): Promise<Record<string, RepoActivity>> {
  try {
    return JSON.parse(await readFile(CACHE_PATH, 'utf8')) as Record<string, RepoActivity>
  } catch {
    return {}
  }
}
 ...
 */
export async function getActivity(repos: string[]): Promise<Record<string, RepoActivity>> {
  if (memo) return memo
  ...
  await Promise.all(
    repos.map(async (repo) => {
      ...
      const mine = rawCommits.filter(
        (c) => !c.author || c.author.login.toLowerCase() === GITHUB_LOGIN.toLowerCase(),
      )
      ...
      out[repo] = {
        ...
        recentCommitCount: dates.filter((d) => now - +new Date(d) < MS_PER_90_DAYS).length,
